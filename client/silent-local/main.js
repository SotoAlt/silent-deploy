// silent-local — client
// Same UI as the canonical silent client, but the JEPA predator runs in
// THIS browser via TF.js. Server stays env-only — sends us the audio_obs
// each frame, we send back predator_action.

const BUILD_TAG = 'silent-local-0.1';
console.log('[silent-local] build =', BUILD_TAG);
...
jepa.init(WEIGHTS_URL).then(() => {
  ...
  console.error('[silent-local] JEPA init failed:', e);
});
...
function handleServer(msg) {
  ...
  // silent-local: server is in client_jepa mode → it sends the audio_obs
  // mel-spec each frame. Feed it to the in-browser JEPA so the next
  // player_action send can attach a fresh predator_action.
  if (msg.audio_obs_b64 && msg.audio_obs_shape) {
    jepa.onObs(msg.audio_obs_b64, msg.audio_obs_shape);
  }
